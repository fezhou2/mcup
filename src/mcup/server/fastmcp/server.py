...
from mcup.server.auth.middleware.auth_context import AuthContextMiddleware
from mcup.server.auth.middleware.bearer_auth import BearerAuthBackend, RequireAuthMiddleware
from mcup.server.auth.provider import OAuthAuthorizationServerProvider, ProviderTokenVerifier, TokenVerifier
from mcup.server.auth.settings import AuthSettings
from mcup.server.elicitation import ElicitationResult, ElicitSchemaModelT, elicit_with_validation
from mcup.server.fastmcp.exceptions import ResourceError
from mcup.server.fastmcp.prompts import Prompt, PromptManager
from mcup.server.fastmcp.resources import FunctionResource, Resource, ResourceManager
from mcup.server.fastmcp.tools import Tool, ToolManager
from mcup.server.fastmcp.utilities.logging import configure_logging, get_logger
from mcup.server.lowlevel.helper_types import ReadResourceContents
from mcup.server.lowlevel.server import LifespanResultT
from mcup.server.lowlevel.server import Server as MCPServer
from mcup.server.lowlevel.server import lifespan as default_lifespan
from mcup.server.session import ServerSession, ServerSessionT
from mcup.server.sse import SseServerTransport
from mcup.server.stdio import stdio_server
from mcup.server.streamable_http import EventStore
...
from mcup.shared.context import LifespanContextT, RequestContext, RequestT
from mcup.types import AnyFunction, ContentBlock, GetPromptResult, ToolAnnotations
from mcup.types import Prompt as MCPPrompt
from mcup.types import PromptArgument as MCPPromptArgument
from mcup.types import Resource as MCPResource
from mcup.types import ResourceTemplate as MCPResourceTemplate
from mcup.types import Tool as MCPTool
...
class FastMCP(Generic[LifespanResultT]):
    ...
    def sse_app(self, mount_path: str | None = None) -> Starlette:
        ...
        if self.settings.auth and self.settings.auth.resource_server_url:
            from mcup.server.auth.routes import create_protected_resource_routes
    ...
    def streamable_http_app(self) -> Starlette:
        ...
        # Add protected resource metadata endpoint if configured as RS
        if self.settings.auth and self.settings.auth.resource_server_url:
            from mcup.server.auth.handlers.metadata import ProtectedResourceMetadataHandler
            from mcup.server.auth.routes import cors_middleware
            from mcup.shared.auth import ProtectedResourceMetadata
